...
- **Translatable text** lives in the central `tenant_texts` table (migration 076), keyed by `(level, reference_id, slot, language)`. Levels: `tenant`, `product`, `characteristic`, `characteristic_value`, `pricing_formula` (formula names — migration 082). The plain `name` column on each entity stays as the EN-canonical fallback. Edit via the `I18nEditor` component → `setEntityI18nText`; read via `resolveText`/`resolveTextI18n` (admin) or `pickTranslation`/`textsToI18n` (widget). Adding a new level requires extending the `tenant_texts_level_check` CHECK, the `TextLevel` type, `TEXT_SLOTS`, and the `textSlots.ts` catalogue.
