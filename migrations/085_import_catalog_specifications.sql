-- Migration 085: extend import_catalog RPC with a Specifications sheet.
--
-- The Specifications sheet carries multi-line / spec-type text blocks for
-- products, characteristics, and characteristic_values, with language as a
-- column (rather than the inline _en / _sr columns used for name and
-- description on the entity sheets).
--
-- Slot rules (enforced by the client parser AND here for safety):
--   product:              specification | product | note | terms (multi-row, sort_order matters)
--   characteristic:       specification (single-row, sort_order = 0)
--   characteristic_value: specification (single-row, sort_order = 0)
--
...
BEGIN
  ...
  LOOP
    ...
    -- Resolve the reference into the inserted UUID for this run.
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Specification already exists for % "%", slot "%", language "%"',
        ...
        USING ERRCODE = '23505',
              DETAIL = json_build_object(
                'code', 'conflict', 'sheet', 'Specifications',
                'key',  v_key, 'slot', v_slot, 'language', v_language
              )::text;
    END IF;
