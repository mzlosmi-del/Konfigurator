  ...
  async function handleInlineEdit(
    ...
    const updated =
      field === 'price_modifier'
        ? { price_modifier: parseFloat(raw) || 0 }
        : { label: raw }

    try {
      const result = await updateCharacteristicValue(value.id, updated)
        ...
        >
          ...
          <input
            ...
            onBlur={e => {
              if (e.target.value !== value.label) {
                handleInlineEdit(value, 'label', e.target.value)
              }
            }}
          />
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <input
              ...
              onBlur={e => {
                if (parseFloat(e.target.value) !== value.price_modifier) {
                  handleInlineEdit(value, 'price_modifier', e.target.value)
                }
              }}
            />
          </div>
