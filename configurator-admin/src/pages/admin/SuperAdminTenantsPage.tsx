...
function StatusBadge({ tenant }: { tenant: TenantOverviewRow }) {
  ...
  if (days === null) {
    return <Badge variant="secondary">{t('No expiry')}</Badge>
  }
  if (days < 0)        return <Badge variant="destructive">{t('Expired {n}d ago').replace('{n}', String(-days))}</Badge>
  if (days <= 14)      return <Badge variant="warning">{t('Expires in {n}d').replace('{n}', String(days))}</Badge>
  return <Badge variant="success">{t('{n}d left').replace('{n}', String(days))}</Badge>
}
...
export function SuperAdminTenantsPage() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title={t('Super admin · Tenants')}
        description={
          <span className="flex items-center gap-1.5">
            <Shield className="h-3.5 w-3.5" />
            {t('Cross-tenant plan, usage, and expiry management.')}
          </span>
        }
      />
      ...
      <div className="p-4 md:p-6 space-y-4">
        <Card>
          <CardContent className="p-3 md:p-4 flex flex-wrap items-end gap-3">
            <div className="space-y-1 flex-1 min-w-[12rem]">
              ...
              <Input
                placeholder={t('Name, slug, or admin email')}
                value={search}
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="font-medium text-sm">{t('No tenants match your filters.')}</p>
            </CardContent>
          </Card>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40">
                    <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('Tenant')}</th>
                    <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('Plan')}</th>
                    <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('Paid until')}</th>
                    <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('Status')}</th>
                    <th className="px-3 py-2 text-right font-medium text-muted-foreground">{t('Products')}</th>
                    ...
                    <tr key={row.id} className="hover:bg-muted/20 transition-colors">
                      <td className="px-3 py-2 min-w-[14rem]">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {row.admin_email ?? <span className="italic">{t('no admin email')}</span>}
                        </div>
                      </td>
                      <td className="px-3 py-2">
                        <Badge variant={PLAN_VARIANT[row.plan]}>{planLabel(row.plan)}</Badge>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs">
                        {formatDate(row.paid_until ?? row.current_period_end)}
                      </td>
                      <td className="px-3 py-2"><StatusBadge tenant={row} /></td>
                      <td className="px-3 py-2 text-right tabular-nums">{row.products_count}</td>
        ...
        <p className="text-xs text-muted-foreground">
          {t('{n} tenants').replace('{n}', String(filtered.length))}
          {filtered.length !== rows.length && t(' (of {n})').replace('{n}', String(rows.length))}
        </p>
      </div>
      ...
      {editing && (
        <TenantEditDialog
          ...
          onError={msg => toast({ title: t('Save failed'), description: msg, variant: 'destructive' })}
          onSuccess={() => toast({ title: t('Plan updated') })}
        />
      )}
...
function TenantEditDialog({ tenant, onClose, onSaved, onError, onSuccess }: TenantEditDialogProps) {
  ...
  async function save() {
    ...
    try {
      ...
      onError(e instanceof Error ? e.message : t('Unknown error'))
    } finally {
      setSaving(false)
    }
  }
      ...
      <DialogContent className="max-w-2xl">
        ...
        {stripeManaged && (
          <div className="flex gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900">
            ...
            <div>
              {t('This tenant is Stripe-managed (subscription')}{' '}
              <span className="font-mono">{tenant.stripe_subscription_id}</span>).
              {' '}{t('Manual plan changes will be overwritten by the next Stripe webhook.')}
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground">{t('Paid until')}</label>
              <div className="flex gap-2">
                ...
                />
                {paidUntil && (
                  <Button variant="ghost" size="sm" onClick={() => setPaidUntil(null)}>{t('Clear')}</Button>
                )}
              </div>
              {days !== null && (
                <p className="text-xs text-muted-foreground">
                  {days < 0 ? t('Expired {n} days ago').replace('{n}', String(-days)) : t('In {n} days').replace('{n}', String(days))}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => bumpDays(30)}>{t('+30 days')}</Button>
            <Button variant="outline" size="sm" onClick={() => bumpDays(90)}>{t('+90 days')}</Button>
            <Button variant="outline" size="sm" onClick={() => bumpDays(365)}>{t('+1 year')}</Button>
            <Button variant="outline" size="sm" onClick={() => bumpDays(-30)}>{t('−30 days')}</Button>
          </div>

          <div className="rounded-md border bg-muted/20 p-3 space-y-2">
            <div className="text-xs font-medium text-muted-foreground">
              {t('Current-period usage vs {plan} limits').replace('{plan}', planLabel(plan))}
            </div>
            <UsageLine label={t('Products')}        used={tenant.products_count}       max={limits?.products_max} />
            <UsageLine label={t('Inquiries (mo)')}  used={tenant.inquiries_this_month} max={limits?.inquiries_per_month} />
          ...
          <div className="rounded-md border p-3 text-xs space-y-1">
            <div className="grid grid-cols-[10rem_1fr] gap-1">
              <span className="text-muted-foreground">{t('Stripe customer')}</span>
              ...
              <span className="text-muted-foreground">{t('Cancel at period end')}</span>
              <span>{tenant.cancel_at_period_end ? t('yes') : t('no')}</span>
